...
print(f'Wrapped {len(results)} result(s).')
"
```
...
| Wrap step fails with unicode error | Write the Python script via `create_file` to `/tmp/wrap.py`, then run it with `python3 /tmp/wrap.py` |
